...
const WEEK_VARIANCE = [1.0,0.92,1.08,0.96,1.12,0.88,1.05,0.98,1.15,0.90,1.03,0.95,1.10,0.85];
...
function calcRoles(rev, hasKitchen, servesAlcohol) {
  ...
  if (rev < 1500) return {roles:[{role:"All-rounder",count:2}],total:2,note:"Two people covering all bases"};
  if (rev < 3000) {
  ...
  if (rev < 5000) {
    if (hasKitchen) return {roles:[{role:"Kitchen",count:1},{role:"Coffee",count:1},{role:"Floor",count:2}],total:4,note:"Full day service team"};
  ...
  if (rev < 7000) {
    return {
      ...
      total:(hasKitchen?2:0)+1+2+(servesAlcohol?1:0),note:"Big day — all stations busy"
    };
  }
  return {
    roles:[...(hasKitchen?[{role:"Kitchen",count:2}]:[]),{role:"Coffee",count:2},{role:"Floor",count:3},...(servesAlcohol?[{role:"Bar",count:1}]:[])],
    total:(hasKitchen?2:0)+2+3+(servesAlcohol?1:0),note:"Peak trading — all hands on deck"
  };
}

// ─── SHIFT TIMING ─────────────────────────────────────────────────────────────
function calcShifts(roles, tradingHours, day) {
  const h = tradingHours[day];
  if (!h || !h.open) return [];

  const open = h.openTime;
  const close = h.closeTime;
  const hasDinner = h.hasDinner;
  const dinnerOpen = h.dinnerOpen;
  const dinnerClose = h.dinnerClose;
  ...
  roles.roles.forEach(({role, count}) => {
    for (let i=0; i<count; i++) {
      if (role === "Kitchen") {
        if (hasDinner) {
          // Split kitchen — one for day, one covers dinner
          if (i === 0) shifts.push({role, start:open, end:close, label:`${role} (day)`});
          else shifts.push({role, start:dinnerOpen-1, end:dinnerClose, label:`${role} (dinner)`});
        } else {
          shifts.push({role, start:open, end:close, label:role});
        }
      } else if (role === "Coffee") {
        // Coffee peaks morning and lunch
        if (i === 0) shifts.push({role, start:open, end:Math.min(close, open+5), label:"Coffee"});
        else shifts.push({role, start:open+2, end:close, label:"Coffee"});
        ...
        if (count === 1) {
          ...
          shifts.push({role, start:open, end:hasDinner?Math.floor((close+dinnerOpen)/2):close, label:"Floor (opening)"});
        } else if (i === 1) {
          shifts.push({role, start:open+2, end:hasDinner?dinnerClose:close, label:"Floor (mid)"});
        } else {
          shifts.push({role, start:hasDinner?dinnerOpen:Math.floor((open+close)/2), end:hasDinner?dinnerClose:close, label:"Floor (close)"});
        }
      } else if (role === "Bar") {
...
function calcDay(day, baseRev, hasKitchen, servesAlcohol, tradingHours, weatherMult=1.0, eventMult=1.0, weekVar=1.0) {
  const h = tradingHours[day];
  ...
  const adj = baseRev * DAY_MULT[day] * weatherMult * eventMult * weekVar;
  const laborBudget = adj * 0.30;
  const totalHours = laborBudget / 29;
  const curve = new Array(24).fill(0);
  const open=h.openTime, close=h.closeTime;
  const dinnerOpen=h.hasDinner?h.dinnerOpen:null, dinnerClose=h.hasDinner?h.dinnerClose:null;

  for(let i=0;i<24;i++){
    const inDay=i>=open&&i<close;
    const inDinner=dinnerOpen&&i>=dinnerOpen&&i<dinnerClose;
    if(inDay||inDinner){
      if(i>=12&&i<=13) curve[i]=1.0;
      else if(i>=18&&i<=20&&(inDinner||inDay)) curve[i]=inDinner?1.0:0.8;
      else if(i>=9&&i<=11) curve[i]=0.7;
  ...
  const shifts=calcShifts(roles,tradingHours,day);
  return {adj,laborBudget,byHour,roles,shifts,closed:false};
}
...
// ─── MICRO COMPONENTS ────────────────────────────────────────────────────────
function Label({text}){return<p style={{fontSize:11,letterSpacing:"0.1em",color:B.warmGrey,fontFamily:"system-ui,-apple-system,sans-serif",textTransform:"uppercase",fontWeight:600,marginBottom:8}}>{text}</p>;}

function Stepper({value,onChange,min=0,max=100,step=1,prefix="",suffix=""}){
  return(
    <div style={{display:"flex",alignItems:"center",gap:16}}>
      <button onClick={()=>onChange(Math.max(min,value-step))} style={{width:40,height:40,borderRadius:"50%",border:`1.5px solid ${B.midGrey}`,background:B.white,color:B.nearBlack,fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 1px 3px rgba(28,21,16,0.08)"}}>−</button>
      ...
      <button onClick={()=>onChange(Math.min(max,value+step))} style={{width:40,height:40,borderRadius:"50%",border:`1.5px solid ${B.midGrey}`,background:B.white,color:B.nearBlack,fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 1px 3px rgba(28,21,16,0.08)"}}>+</button>
    </div>
  );
}
  ...
  return(
    <div style={{display:"flex",gap:10}}>
      {[yes,no].map(opt=>{
        ...
        return<button key={opt} onClick={()=>onChange(opt===yes)} style={{flex:1,padding:"13px 0",borderRadius:14,fontSize:15,fontWeight:600,cursor:"pointer",fontFamily:"system-ui,-apple-system,sans-serif",border:`1.5px solid ${active?B.amber:B.midGrey}`,background:active?B.amberLight:B.white,color:active?B.amberDark:B.warmGrey,transition:"all 0.15s",boxShadow:active?`0 2px 8px rgba(232,160,32,0.2)`:"none"}}>{opt}</button>;
      })}
    </div>
  );
}
...
function ShiftRow({shift}){
  ...
  return(
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:c.bg,borderRadius:10,padding:"11px 14px",marginBottom:8}}>
      ...
      <span style={{fontSize:13,fontWeight:600,color:c.text,fontFamily:"system-ui,-apple-system,sans-serif",opacity:0.85}}>
...
// ─── FEEDBACK COMPONENT ───────────────────────────────────────────────────────
function FeedbackPanel({dayKey, feedback, onSave}) {
  const[selected,setSelected]=useState(feedback?.outcome||null);
  ...
  function save(outcome){
    setSelected(outcome);
    setSaved(true);
    onSave({outcome,note,savedAt:new Date().toISOString()});
  }
  ...
  if(saved&&selected){
    ...
    return(
      <div style={{background:opt.bg,borderRadius:14,padding:"12px 16px",
        display:"flex",alignItems:"center",gap:10}}>
        <span style={{fontSize:18}}>{opt.icon}</span>
        <div>
          <p style={{fontSize:13,fontWeight:600,color:opt.color,
            fontFamily:"system-ui,-apple-system,sans-serif"}}>{opt.label}</p>
          {note&&<p style={{fontSize:12,color:B.warmGrey,marginTop:2,
            fontFamily:"system-ui,-apple-system,sans-serif"}}>{note}</p>}
        </div>
        <button onClick={()=>setSaved(false)} style={{marginLeft:"auto",fontSize:12,
          color:B.warmGrey,background:"none",border:"none",cursor:"pointer",
          fontFamily:"system-ui,-apple-system,sans-serif"}}>Edit</button>
      </div>
    );
  }
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
        {options.map(opt=>(
          <button key={opt.key} onClick={()=>save(opt.key)} style={{
            display:"flex",alignItems:"center",gap:12,padding:"12px 14px",
            borderRadius:12,border:`1.5px solid ${selected===opt.key?opt.color:B.midGrey}`,
            background:selected===opt.key?opt.bg:B.white,
      ...
      <textarea
        placeholder="Optional notes — e.g. 'AFL crowd came in at 6pm'"
        value={note}
        onChange={e=>setNote(e.target.value)}
        style={{width:"100%",padding:"12px 14px",borderRadius:12,
          border:`1.5px solid ${B.midGrey}`,fontSize:14,resize:"none",
          fontFamily:"system-ui,-apple-system,sans-serif",
...
function Onboarding({onComplete}){
  ...
  const steps=[
    ...
    <div key={0} style={{textAlign:"center",paddingTop:32}}>
      ...
      <h1 style={{fontSize:38,fontWeight:700,letterSpacing:"-0.02em",color:B.nearBlack,marginBottom:16,lineHeight:1.15,fontFamily:"system-ui,-apple-system,sans-serif"}}>It's time to<br/><span style={{color:B.amber}}>WageSave.</span></h1>
    ...
    // 1: Venue details
    <div key={1}>
      <p style={{fontSize:13,color:B.warmGrey,marginBottom:4,fontFamily:"system-ui,-apple-system,sans-serif"}}>1 of 4</p>
    ...
    // 2: Trading hours
    <div key={2}>
      <p style={{fontSize:13,color:B.warmGrey,marginBottom:4,fontFamily:"system-ui,-apple-system,sans-serif"}}>2 of 4</p>
      ...
      <p style={{fontSize:15,color:B.warmGrey,marginBottom:24,fontFamily:"system-ui,-apple-system,sans-serif"}}>Set hours per day — closed days show no staff predictions.</p>
      {DAYS.map(day=>{
      ...
      <div style={{marginTop:8}}><AmberBtn label="Next →" onClick={()=>setStep(3)}/></div>
    </div>,

    // 3: Busy periods
    <div key={3}>
      <p style={{fontSize:13,color:B.warmGrey,marginBottom:4,fontFamily:"system-ui,-apple-system,sans-serif"}}>3 of 4</p>
      <h2 style={{fontSize:28,fontWeight:700,color:B.nearBlack,letterSpacing:"-0.02em",marginBottom:8,lineHeight:1.2,fontFamily:"system-ui,-apple-system,sans-serif"}}>When are you<br/>busiest?</h2>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:28}}>
        {[["☀️ Morning","Morning"],["🍽 Lunch","Lunch"],["☕ Afternoon","Afternoon"],["🌙 Dinner","Dinner"],["🍺 Late night","Late night"],["📅 Weekends","Weekends"]].map(([label,val])=>{
          ...
          return<button key={val} onClick={()=>setData(d=>({...d,busyPeriods:active?d.busyPeriods.filter(x=>x!==val):[...d.busyPeriods,val]}))} style={{padding:"14px 12px",borderRadius:14,fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:"system-ui,-apple-system,sans-serif",border:`1.5px solid ${active?B.amber:B.midGrey}`,background:active?B.amberLight:B.white,color:active?B.amberDark:B.warmGrey,transition:"all 0.15s",boxShadow:active?`0 2px 8px rgba(232,160,32,0.2)`:"none"}}>{label}</button>;
        })}
      </div>
      ...
      <AmberBtn label="Next →" onClick={()=>setStep(4)}/>
    </div>,

    // 4: Revenue
    <div key={4}>
      <p style={{fontSize:13,color:B.warmGrey,marginBottom:4,fontFamily:"system-ui,-apple-system,sans-serif"}}>4 of 4</p>
      ...
      <p style={{fontSize:15,color:B.warmGrey,marginBottom:32,lineHeight:1.5,fontFamily:"system-ui,-apple-system,sans-serif"}}>Your baseline. WageSave adjusts for day of week, weather and events.</p>
  ...
  return(
    <div style={{minHeight:"100vh",background:B.amberPale}}>
      <style>{`*{box-sizing:border-box;margin:0;padding:0;}@keyframes fadeUp{from{opacity:0;transform:translateY(16px);}to{opacity:1;transform:translateY(0);}}.fade-up{animation:fadeUp 0.38s cubic-bezier(.23,1,.32,1) forwards;}input[type=range]{-webkit-appearance:none;height:3px;border-radius:2px;background:${B.midGrey};outline:none;}input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:20px;height:20px;border-radius:50%;background:${B.amber};cursor:pointer;box-shadow:0 2px 6px rgba(232,160,32,0.4);}select:focus{outline:none;}`}</style>
      {step>0&&<div style={{height:3,background:B.lightGrey}}><div style={{height:"100%",background:B.amber,borderRadius:"0 2px 2px 0",width:`${(step/4)*100}%`,transition:"width 0.4s ease"}}/></div>}
      <div style={{maxWidth:440,margin:"0 auto",padding:"40px 24px 80px"}}>
        {step===0&&<p style={{fontSize:14,fontWeight:700,color:B.amber,letterSpacing:"0.1em",textTransform:"uppercase",textAlign:"center",marginBottom:48,fontFamily:"system-ui,-apple-system,sans-serif"}}>WageSave</p>}
        {step>0&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:40}}><p style={{fontSize:15,fontWeight:700,color:B.amber,fontFamily:"system-ui,-apple-system,sans-serif",letterSpacing:"0.06em"}}>WageSave</p>{step>1&&<button onClick={()=>setStep(s=>s-1)} style={{fontSize:13,color:B.warmGrey,background:"none",border:"none",cursor:"pointer",fontFamily:"system-ui,-apple-system,sans-serif"}}>← Back</button>}</div>}
        ...
        <div style={{padding:"12px 24px 48px"}}>
          ...
          {closed?(
            <div style={{textAlign:"center",padding:"32px 0"}}>
              ...
              <p style={{fontSize:14,color:B.warmGrey,marginTop:8,fontFamily:"system-ui,-apple-system,sans-serif"}}>No staffing needed.</p>
            </div>
          ):(
            <>
              {/* Hero */}
              ...
              <div style={{background:B.amberPale,borderRadius:14,padding:16,marginBottom:20}}>
                ...
                {actual>0&&(
                  <div style={{marginTop:16,paddingTop:16,borderTop:`1px solid ${B.lightGrey}`}}>
                    ...
                    :diff<0?<p style={{fontSize:14,color:B.success,fontWeight:600,fontFamily:"system-ui,-apple-system,sans-serif"}}>✓ Understaffed by {Math.abs(diff)} — revenue at risk</p>
                    :<p style={{fontSize:14,color:B.success,fontWeight:600,fontFamily:"system-ui,-apple-system,sans-serif"}}>✓ Staffing looks right</p>}
                  </div>
                )}
              </div>

              {/* Feedback — only show for past or today */}
              {isPast&&(
                <div style={{background:B.amberPale,borderRadius:14,padding:16}}>
                  <FeedbackPanel
                    dayKey={`${day}-${dateKey(date)}`}
                    feedback={feedback}
                    onSave={onFeedback}/>
                </div>
              )}
...
function MainApp({venue}){
  ...
  const weekData=DAYS.map((day,i)=>{
    ...
    const dayData=calcDay(day,baseRevenue,venue.hasKitchen,venue.servesAlcohol,venue.tradingHours,weatherMult,eventMult,weekVar);
    ...
    <div style={{minHeight:"100vh",background:B.amberPale,paddingBottom:80}}>
      ...
      <div style={{background:B.white,borderBottom:`1px solid ${B.lightGrey}`,padding:"14px 20px",position:"sticky",top:0,zIndex:100,boxShadow:"0 1px 8px rgba(28,21,16,0.06)"}}>
        <div style={{maxWidth:480,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <p style={{fontSize:13,fontWeight:700,color:B.amber,letterSpacing:"0.08em",textTransform:"uppercase",fontFamily:"system-ui,-apple-system,sans-serif"}}>WageSave</p>
            <p style={{fontSize:12,color:B.warmGrey,fontFamily:"system-ui,-apple-system,sans-serif"}}>{venue.name} · {venue.suburb}</p>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"20px 18px 0"}}>
        ...
        {/* CSV nudge banner */}
        {showCsvNudge&&(
          <div style={{background:B.amberLight,borderRadius:14,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"space-between",border:`1px solid ${B.midGrey}`}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <button onClick={()=>setShowCsvNudge(false)} style={{fontSize:16,color:B.warmGrey,background:"none",border:"none",cursor:"pointer",lineHeight:1}}>×</button>
            </div>
          </div>
        )}
        ...
        <div style={{marginBottom:20}}>
          ...
          <h1 style={{fontSize:26,fontWeight:700,letterSpacing:"-0.02em",lineHeight:1.2,color:B.nearBlack,fontFamily:"system-ui,-apple-system,sans-serif"}}>It's time to WageSave<br/><span style={{color:B.amber}}>at {venue.name}.</span></h1>
        </div>
          ...
          {weekData.map(({day,date,roles,adj,flags,byHour,closed},idx)=>{
            ...
            return(
              <div key={day} className="card-in"
                ...
                style={{background:closed?"#F8F5F0":B.white,borderRadius:20,padding:18,cursor:"pointer",border:`1.5px solid ${isToday?B.amber:B.lightGrey}`,boxShadow:isToday?`0 4px 20px rgba(232,160,32,0.25)`:`0 2px 8px rgba(28,21,16,0.05)`,opacity:isPast?0.55:1,transition:"transform 0.12s ease,box-shadow 0.12s ease",animationDelay:`${idx*0.05}s`,position:"relative"}}
                ...
                {closed?(
                  ...
                  <>
                    ...
                    {fb&&(
                      <div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${B.lightGrey}`,fontSize:11,fontFamily:"system-ui,-apple-system,sans-serif",color:fb.outcome==="right"?B.success:fb.outcome==="over"?B.danger:"#E65100"}}>
                        {fb.outcome==="right"?"✅ Was right":fb.outcome==="over"?"⬆️ Sent home early":"⬇️ Got slammed"}
                      </div>
                    )}
